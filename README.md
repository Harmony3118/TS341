...
## About outputs
The script `yolo_plus_imm.py` will generate a .json file with the name of the video
and the suffix `_yolo.mp4.json`. It contains the predicted position for each frame,
that can be used to check tracker evaluation.

To run tracker evaluation, run `poetry run evaluate`, select the reference video used,
and select the ground truth data. The ground truth must be a JSON file with a single array
containing tuples of x,y coordinates of a single drone for each frame.

Error is computed as a L2 norm. See the example JSON file in the `videos/` folder.